...
- **sitemap.xml**: 动态生成，包含 12 个静态页 + 所有已发布博客 + 所有 Verified 教授公开页 (professor/[slug])。教授查询使用 .range() 分页拉取（每批 1000 行循环）绕过 Supabase PostgREST 1000 行上限，确保全部 ~7888 位教授收录
...
- [x] Migration: DROP TABLE IF EXISTS public.user_credits（无 CASCADE，0 行数据，0 外键依赖）
