# 推广中心海报编辑器设计规格
> 版本: V1.3 | 日期: 2026-05-24 | 状态: 确定稿,不可随意覆盖

## 核心原则
Canvas 尺寸固定,图片适应 canvas,不是 canvas 适应图片。

## 技术方案
纯 HTML5 Canvas 2D API,不使用 fabric.js。
React useEffect 监听所有配置状态,自动触发全量重绘。
预览容器 CSS: width:100%, object-fit:contain,canvas 内部保持 1080px 分辨率。
导出: canvas.toDataURL('image/png') 直接下载,预览和导出同一个 canvas,零差异。
...
### 左侧面板顺序（从上到下）
1. 主标题输入 + 字号下拉(36/42/48/54/60px,默认48) + 颜色选择(白/黑/金#D4A843)
2. 副标题输入 + 字号下拉(18/22/24/28/32px,默认24)
3. 卖点文字: 4个可编辑输入框 + 共享字号下拉(16/18/20/22px,默认20)
4. 添加文字按钮（QR 是预设元素，不需手动添加按钮）
5. 渠道下拉 / 字体下拉
6. 布局预设: 紧凑/标准/宽松 三按钮（控制元素间距）
7. 显示开关 checkbox: 二维码/网址文字/邀请码/渠道标识
8. 尺寸切换 3:4/1:1/9:16
...
## 布局预设（两个海报 tab 共用）
- 紧凑: titleY=0.11, lineExtra=4, titleSubGap=4, subPtGap=24, ptGap=28（内容多时使用）
- 标准: titleY=0.15, lineExtra=14, titleSubGap=8, subPtGap=50, ptGap=40（默认）
- 宽松: titleY=0.19, lineExtra=24, titleSubGap=20, subPtGap=70, ptGap=50（简约风）
- 切换主题时颜色自动适配: 深色主题默认白字,简约白底默认深灰字（用户仍可在颜色选择器改）
