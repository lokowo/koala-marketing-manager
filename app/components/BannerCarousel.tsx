...
export default function BannerCarousel({ heroMode = false }: { heroMode?: boolean } = {}) {
  ...
  if (banners.length === 0) {
    ...
    return (
      <div className="relative overflow-hidden rounded-xl aspect-[3/2] bg-gradient-to-br from-[#f0ebe0] to-[#e2d9c6] dark:from-[#1a2a20] dark:to-[#162028] flex items-center justify-center">
      ...
      >
        ...
        >
          {banners.map((banner, i) => (
            <div
              key={banner.id}
              className={`w-full flex-shrink-0 relative ${heroMode ? 'aspect-[3/2]' : 'h-[180px] md:h-[280px] lg:h-[380px]'} ${clickable(banner) ? 'cursor-pointer' : ''}`}
