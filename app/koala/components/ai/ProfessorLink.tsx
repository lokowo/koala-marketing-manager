...
export function ProfessorLink({
  ...
  const color = matchScore ? (matchScore >= 75 ? '#5a8060' : matchScore >= 50 ? '#D4A843' : '#b06040') : '#D4A843';

  if (compact) {
    return (
      <Link
        href={`/koala/professors/${professorId}`}
        className="flex items-center gap-2 rounded-xl px-2.5 py-1.5 no-underline bg-[#D4A843]/[0.06] border border-[#D4A843]/10"
      >
          ...
          {researchTags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1.5">
              {researchTags.slice(0, 3).map(tag => (
                <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded-full bg-[#D4A843]/[0.06] text-[#D4A843]">
                  {tag}
                </span>
              ))}
            </div>
