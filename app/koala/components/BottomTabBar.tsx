...
export default function BottomTabBar() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 bg-white dark:bg-[#0a0e14] border-t border-gray-200 dark:border-[rgba(201,169,110,0.12)]" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="text-center text-[9px] text-[#D4A843]/30 pt-0.5">{APP_VERSION}</div>
      <div className="relative flex justify-around items-end px-4 pb-6 pt-2 max-w-[480px] mx-auto">
        {LEFT_TABS.map(tab => {
          ...
          return (
            <Link key={tab.href} href={tab.href} className="flex flex-col items-center flex-1 gap-1 no-underline">
              <Icon className="size-5" style={{ color: active ? '#D4A843' : undefined }} strokeWidth={active ? 2.2 : 1.5} />
              <span className={`text-[10px] tracking-wide ${active ? 'text-[#D4A843] font-semibold' : 'text-gray-400 dark:text-[#5a6a6e]'}`}>{tab.label}</span>
            </Link>
          );
        })}
        <div className="flex justify-center flex-1">
          <Link
            href={koalaHref}
            className="size-14 rounded-full flex absolute -top-5 flex-col justify-center items-center no-underline border-2 border-[#D4A843]/30"
            style={{
              background: koalaActive
                ? 'linear-gradient(135deg, #D4A843, #a68540)'
                : 'linear-gradient(135deg, #1a3a2a, #0d2818)',
              boxShadow: koalaActive ? '0 6px 20px rgba(212,168,67,0.4)' : '0 4px 12px rgba(0,0,0,0.3)',
            }}
          >
            <span className="text-xl leading-none">🐨</span>
            <span className={`font-semibold text-[9px] ${koalaActive ? 'text-[#0a0e14]' : 'text-[#D4A843]'}`}>Koala</span>
          </Link>
        </div>
        {RIGHT_TABS.map(tab => {
          const active = isActive(tab.href);
