    ...
    <div className="fixed inset-x-0 bottom-0 z-50" style={{ backgroundColor: '#0a0e14', borderTop: '1px solid rgba(201,169,110,0.12)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="relative flex justify-around items-end px-4 pb-6 pt-2" style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        <div className="flex justify-center flex-1">
          <Link href="/koala/chat" className="size-14 rounded-full flex absolute -top-5 flex-col justify-center items-center" style={{ background: chatActive ? 'linear-gradient(135deg, #c9a96e, #a68540)' : 'linear-gradient(135deg, #1a3a2a, #0d2818)', border: '2px solid rgba(201,169,110,0.3)', boxShadow: chatActive ? '0 6px 20px rgba(201,169,110,0.4)' : '0 4px 12px rgba(0,0,0,0.3)', textDecoration: 'none' }}>
            <span className="text-xl leading-none">🐨</span>
            <span className="font-semibold text-[9px]" style={{ color: chatActive ? '#0a0e14' : '#c9a96e' }}>Koala</span>
          </Link>
        </div>
        {RIGHT_TABS.map(tab => {
          const active = isActive(tab.href);
