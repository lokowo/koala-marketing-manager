...
export default function FAQManagementPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {seedResult && (
        <div className="mb-4 p-3 text-sm bg-blue-50 text-blue-700 rounded-lg">{seedResult}</div>
      )}
        ...
        {testResult && (
          <div className="mt-3 p-3 bg-green-50 rounded-lg text-sm">
            <div className="flex items-center gap-2 mb-2">
              <span className="font-medium text-green-800">命中!</span>
              <span className="px-2 py-0.5 bg-green-200 text-green-800 rounded text-xs">{testResult.category}</span>
              <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs">匹配分: {(testResult.score * 100).toFixed(0)}%</span>
            </div>
            <p className="text-gray-700 whitespace-pre-wrap">{testResult.answer_zh}</p>
          </div>
        )}
        {testNoMatch && (
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {faqs.map(faq => (
                <tr key={faq.id} className={`hover:bg-gray-50 ${!faq.enabled ? 'opacity-50' : ''}`}>
                  <td className="px-4 py-3">
                    <span className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded text-xs font-medium">{faq.category}</span>
                  </td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-4 space-y-4">
              <div>
                ...
                <input value={formCategory} onChange={e => setFormCategory(e.target.value)} placeholder="如：pricing, credits, usage" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
              </div>
              <div>
                <label className="block text-xs text-gray-600 mb-1">关键词 *（逗号分隔）</label>
                <input value={formKeywords} onChange={e => setFormKeywords(e.target.value)} placeholder="价格, 多少钱, pricing, cost" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
              </div>
              <div>
                <label className="block text-xs text-gray-600 mb-1">中文答案 *</label>
                <textarea value={formAnswerZh} onChange={e => setFormAnswerZh(e.target.value)} rows={4} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 resize-vertical" />
              </div>
              <div>
                <label className="block text-xs text-gray-600 mb-1">英文答案 *</label>
                <textarea value={formAnswerEn} onChange={e => setFormAnswerEn(e.target.value)} rows={4} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 resize-vertical" />
              </div>
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="block text-xs text-gray-600 mb-1">Rich Card 类型（可选）</label>
                  <input value={formRichCardType} onChange={e => setFormRichCardType(e.target.value)} placeholder="如：pricing_card" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
                <div className="w-24">
                  <label className="block text-xs text-gray-600 mb-1">优先级</label>
                  <input type="number" value={formPriority} onChange={e => setFormPriority(Number(e.target.value))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
              </div>
            </div>
