...
export default function PublishingPage() {
  ...
  if (loading) return <div className="p-6 text-gray-500">Loading publishing records...</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white rounded-lg shadow p-6">
          <p className="text-gray-600 text-sm">{tp.totalViews}</p>
          <p className="text-3xl font-bold text-gray-900">{stats?.totalViews.toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <p className="text-gray-600 text-sm">{tp.totalDMs}</p>
          <p className="text-3xl font-bold text-blue-600">{stats?.totalDMs}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <p className="text-gray-600 text-sm">{tp.totalConsultations}</p>
          <p className="text-3xl font-bold text-green-600">{stats?.totalConsultations}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <p className="text-gray-600 text-sm">{tp.bestPlatform}</p>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {records.map((record) => (
                <tr key={record.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{record.platform}</td>
                  <td className="px-4 py-3 text-sm text-gray-700">{record.contentTitle}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{record.publishDate}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-semibold">{record.views.toLocaleString()}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{record.likes}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{record.saves}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{record.comments}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-blue-600 font-medium">{record.dms}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{record.wechatAdds}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-green-600 font-medium">{record.consultations}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{record.conversionNotes}</td>
                </tr>
              ))}
            </tbody>
          </table>
