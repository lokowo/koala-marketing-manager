  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Ola 触发规则</h1>
          <p className="text-sm text-gray-500 mt-1">管理 Ola AI 主动触发气泡规则</p>
        </div>
        <div className="flex gap-2">
          <button onClick={handleSeed} disabled={seeding} className="px-3 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50">
            {seeding ? '初始化中...' : '🌱 初始化种子'}
          </button>
      ...
      {seedResult && <div className="mb-4 p-3 text-sm bg-blue-50 text-blue-700 rounded-lg">{seedResult}</div>}
            ...
            <tbody className="divide-y divide-gray-100">
              {triggers.map(t => (
                <tr key={t.id} className={`hover:bg-gray-50 ${!t.enabled ? 'opacity-50' : ''}`}>
                  <td className="px-4 py-3 font-mono text-xs text-gray-600">{t.trigger_key}</td>
                  <td className="px-4 py-3"><span className="px-2 py-0.5 bg-gray-100 rounded text-xs">{t.page}</span></td>
                  <td className="px-4 py-3 max-w-[250px] truncate text-gray-700">{t.message_zh}</td>
                  <td className="px-4 py-3 text-center text-gray-500">{t.priority}</td>
                  <td className="px-4 py-3 text-center">
                    <button onClick={() => handleToggle(t)} className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${t.enabled ? 'bg-green-500' : 'bg-gray-300'}`}>
                      <span className={`inline-block h-3.5 w-3.5 rounded-full bg-white transition-transform ${t.enabled ? 'translate-x-4.5' : 'translate-x-1'}`} />
                    </button>
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="font-semibold text-gray-900">{editing ? '编辑触发规则' : '新增触发规则'}</h3>
            </div>
            <div className="px-6 py-4 space-y-3">
              <div>
                <label className="block text-xs text-gray-600 mb-1">Trigger Key *</label>
                <input value={form.trigger_key} onChange={e => setForm(f => ({ ...f, trigger_key: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="new_user_welcome" />
              </div>
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="block text-xs text-gray-600 mb-1">页面 *</label>
                  <input value={form.page} onChange={e => setForm(f => ({ ...f, page: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="home / * / professors/[id]" />
                </div>
                <div className="w-24">
                  <label className="block text-xs text-gray-600 mb-1">优先级</label>
                  <input type="number" value={form.priority} onChange={e => setForm(f => ({ ...f, priority: Number(e.target.value) }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-gray-600 mb-1">中文消息 *</label>
                <textarea value={form.message_zh} onChange={e => setForm(f => ({ ...f, message_zh: e.target.value }))} rows={2} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 resize-vertical" />
              </div>
              <div>
                <label className="block text-xs text-gray-600 mb-1">英文消息 *</label>
                <textarea value={form.message_en} onChange={e => setForm(f => ({ ...f, message_en: e.target.value }))} rows={2} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 resize-vertical" />
              </div>
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="block text-xs text-gray-600 mb-1">Ola 状态</label>
                ...
                <div className="flex-1">
                  <label className="block text-xs text-gray-600 mb-1">动作类型</label>
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-2">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 text-sm text-gray-600">取消</button>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">确定删除触发规则「{deleteTarget.trigger_key}」？</p>
