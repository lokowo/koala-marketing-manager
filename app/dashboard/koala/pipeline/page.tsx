...
export default function PipelinePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-light tracking-tight text-gray-900 dark:text-gray-100">采集管线监控</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">监控教授数据自动采集状态</p>
      </div>
      ...
      <div className="bg-white dark:bg-[#1E293B] rounded-xl border border-gray-200 dark:border-gray-700 p-4">
        ...
        <div className="space-y-2">
          {apiStatus.map(api => (
            <div key={api.name} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{api.name}</span>
              <div className="flex items-center gap-3">
                <span className="text-xs text-gray-500 dark:text-gray-400">{api.latency}</span>
      ...
      <div className="bg-white dark:bg-[#1E293B] rounded-xl border border-gray-200 dark:border-gray-700 p-4">
        ...
        {log.length > 0 && (
          <div className="bg-gray-50 dark:bg-gray-800/50 rounded p-3 font-mono text-xs space-y-1 max-h-40 overflow-y-auto">
            {log.map((l, i) => <div key={i}>{l}</div>)}
          </div>
        )}
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">手动补充教授：直接在 <a href="/dashboard/koala/professors" className="text-blue-600">教授管理</a> 页面添加</p>
      </div>
    </div>
  );
}
