...
export default function PipelinePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">采集管线监控</h1>
        <p className="text-sm text-gray-500 mt-1">监控教授数据自动采集状态</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <h2 className="text-lg font-semibold text-gray-900 mb-3">API 状态</h2>
        <div className="space-y-2">
          {apiStatus.map(api => (
            <div key={api.name} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-gray-900">手动触发采集</h2>
          <button
            onClick={triggerSync}
            disabled={triggering}
        ...
        <p className="text-xs text-gray-500 mt-2">手动补充教授：直接在 <a href="/dashboard/koala/professors" className="text-blue-600">教授管理</a> 页面添加</p>
      </div>
    </div>
  );
}
