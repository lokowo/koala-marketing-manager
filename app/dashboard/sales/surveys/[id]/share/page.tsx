...
export default function SalesSharePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    fetch(`/api/surveys/${id}`).then(r => r.ok ? r.json() : null).then(surveyData => {
      setSurvey(surveyData);
      if (surveyData) fetchLinks();
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/dashboard/sales/surveys')} className="text-sm text-slate-400 hover:text-slate-600">&larr; 返回</button>
      ...
      <div className="flex gap-3">
        <Link
          href={`/dashboard/sales/surveys/${id}/clients`}
