...
export default function SalesClientsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [surveyRes, responsesRes] = await Promise.all([
      ...
      fetch(`/api/surveys/responses?survey_id=${surveyId}&page=${page}&limit=30&status=completed`).then(r => r.json()),
    ]);
    setSurveyTitle(surveyRes.title || '');
    setQuestions(surveyRes.questions || []);
    const allResponses: SurveyResponse[] = responsesRes.responses || [];
  ...
  function isIncomplete(r: SurveyResponse): boolean {
    const meta = r.metadata || {};
    return !meta.respondent_name || !meta.respondent_email || !meta.respondent_phone;
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold text-slate-800">{surveyTitle} — 客户详情</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-xl border border-slate-200">
          ...
          <p className="text-slate-500 text-sm">暂无已完成的回复</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 text-left">
                  ...
                  <th className="px-4 py-3 text-slate-500 font-medium">完整度</th>
                ...
                {responses.map((r, i) => {
                  ...
                  return (
                    <tr key={r.id} className={`border-t border-slate-100 cursor-pointer transition-colors ${
                      selectedId === r.id ? 'bg-amber-50' : incomplete ? 'bg-amber-50/30 hover:bg-amber-50/50' : 'hover:bg-slate-50/50'
                      ...
                      <td className="px-4 py-3">
                        {incomplete ? (
                          <span className="px-2 py-0.5 rounded-full text-xs bg-amber-100 text-amber-600">不完整</span>
                        ) : (
                          <span className="px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-600">完整</span>
                        )}
                      </td>
          ...
          {selectedResponse && (
            <div className="w-80 bg-white rounded-xl border border-slate-200 p-5 space-y-4 flex-shrink-0 self-start sticky top-4">
              ...
              <div className="bg-slate-50 rounded-lg p-3 space-y-1">
                ...
                {isIncomplete(selectedResponse) && (
                  <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-xs bg-amber-100 text-amber-600">信息不完整</span>
                )}
