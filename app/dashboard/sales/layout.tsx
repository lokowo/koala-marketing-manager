...
export default function SalesLayout({ children }: { children: ReactNode }) {
  ...
  const sidebarContent = (mobile?: boolean) => (
    <>
      <div className={`${collapsed && !mobile ? 'px-3 py-4' : 'px-5 py-5'} border-b border-[#E2E8F0] flex items-center gap-3`}>
        {(!collapsed || mobile) && (
        ...
        {mobile ? (
          <button onClick={() => setSidebarOpen(false)} className="p-1.5 rounded-lg hover:bg-[#F8FAFC] text-[#94A3B8] hover:text-[#1E293B] transition-colors flex-shrink-0">
            <IconX size={16} strokeWidth={2} />
          </button>
        ) : (
          <button onClick={() => setCollapsed(v => !v)} className="p-1.5 rounded-lg hover:bg-[#F8FAFC] text-[#94A3B8] hover:text-[#1E293B] transition-colors flex-shrink-0">
              ...
              <div key={item.href}>
                ...
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] transition-colors no-underline ${
                    active
                      ? 'font-medium text-[#F59E0B] bg-[#FFFBEB]'
      ...
      <div className={`${collapsed && !mobile ? 'px-2' : 'px-3'} py-3 border-t border-[#E2E8F0] space-y-0.5`}>
        {isAdmin && (
          <Link
            href="/dashboard/koala"
            onClick={() => mobile && setSidebarOpen(false)}
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] text-[#64748B] hover:bg-[#F8FAFC] hover:text-[#1E293B] transition-colors no-underline"
            title="管理后台"
          >
        ...
        <button
          ...
          className="flex items-center gap-3 w-full text-left px-3 py-2.5 rounded-lg text-[13px] text-[#64748B] hover:bg-[#F8FAFC] hover:text-[#1E293B] transition-colors"
          title="退出登录"
        >
  ...
  return (
    <div className="flex h-screen bg-[#F9FAFB]">
      ...
      <div className={`${collapsed ? 'w-16' : 'w-[220px]'} bg-white border-r border-[#E2E8F0] hidden md:flex flex-col shrink-0 transition-all duration-200`}>
        {sidebarContent()}
      </div>

      <div className="flex-1 flex flex-col min-w-0">
        <div className="bg-white border-b border-[#E2E8F0] px-4 py-3 flex items-center gap-3 md:hidden">
          <button onClick={() => setSidebarOpen(true)} className="p-1.5 rounded-lg hover:bg-[#F8FAFC] text-[#64748B]">
