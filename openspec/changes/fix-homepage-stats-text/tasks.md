...
1. Line ~627: Change `认证导师` → `教授·学者·研究员`
2. Line ~630: Change `8` → `30+`
3. Line ~631: Change `Go8 大学` → `澳洲大学`

### Task 2: Update three-step flow copy

**File**: `app/koala/home/HomeClient.tsx`

1. Line ~329: Change `覆盖 8 所 Go8 大学` → `覆盖全澳 30+ 所大学`
2. Line ~338: Change `从 ${profCount} 位导师中` → `从 ${profCount} 位导师/学者中`
